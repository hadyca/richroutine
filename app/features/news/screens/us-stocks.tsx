import type { Route } from "./+types/us-stocks";
...
export const loader = async ({ request }: Route.LoaderArgs) => {
  ...
  // 2. Filter for US market tickers only
  const usTickers = watchlist
  ...
  const [analyses, expertOpinion] = await Promise.all([
    getLatestTickerAnalyses(client, usTickers),
    getLatestStockExpertOpinion(client, userId, "US"),
  ]);
  // 4. Map the analyses with the watchlist data format
  const analyzedTickers = usTickers.map((ticker: string) => {
...
export default function UsStocksScreen({ loaderData }: Route.ComponentProps) {
